...
from Code.utils.split_class_MMViT import split_class
import shutil
import  torch
...
def inference(num_classes, input_channels, snapshot_dir, save_path):
    ...
    lung_model = net.MMViTSeg(classes=3, aux=False).cuda()
    print(lung_model)
    ...
    for index, (img, pseudo, img_mask, name) in enumerate(test_dataloader):
        ...
        imsave(save_path + name[0].replace('.jpg', '.png'), pred)
...
if __name__ == "__main__":
    inference(num_classes=3,
              input_channels=6,
              snapshot_dir='./Snapshots/save_weights/MulCls_MMViTSeg/MulCls_MMViTSeg_200.pkl',
